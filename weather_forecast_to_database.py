...
def run_weather_data():
    ...
    weather_now = get_weather(lat, lon, time_now)
    forecast_list = forecasts(lat, lon, time_now)
    add_to_database(weather_now, forecast_list[0], forecast_list[1], forecast_list[2])

# Get weather information from asked moment
def get_weather(lat, lon, time_now):
...
def forecasts(lat, lon, time_now):
    ...
    if (len(wanted_near_forecast) == 1):
        wanted_near_forecast = "0" + wanted_near_forecast
    ...
    if (response.ok):
        ...
        return [three_hour_forecast, tomorrow_forecast, day_after_tomorrow_forecast]


def add_to_database(weather_now, three_hour_forecast, tomorrow_forecast, day_after_tomorrow_forecast):
